...
// Test with Jane Doe example and similar formats
const testCases = [
  ...
  "JOHN ANDERSON Software Engineer & Problem Solver",
  "Contact: Alice Wilson | alice@email.com",
  "Name: Robert Davis | robert.davis@company.com",
  "Candidate: Emily Taylor | emily.taylor@gmail.com"
];
...
function extractNameSmart(lines) {
  ...
  for (let i = 0; i < Math.min(10, lines.length); i++) {
    ...
    // Ignore lines with emails, phones, URLs, or section headers
    if (/\b(email|phone|contact|address|linkedin|github|curriculum|vitae|cv|resume|summary|profile|objective|education|experience|skills|projects|certifications|awards|references|www\.|http|@|\d{3}-\d{3}-\d{4}|\(\d{3}\)\s*\d{3}-\d{4})\b/i.test(line)) continue;
